...
- inheritance was to be used throughout the design to an even greater extent than is already provided in SIMULA. This enables C++SIM to be more flexible and extensible, allowing new functionality to be added without affecting the overall system structure. For example, our I/O facilities, random number generators and probability distribution functions are entirely object-oriented, relying on inheritance to specialise their behaviour.
...
The scheduler maintains the simulation clock, and the current value of this clock is obtained by invoking the CurrentTime() method. You can use the simulationStarted() method to determine whether or not the simulation is running.
...
More details on resetting the simulation can be found later in the document after we discuss simulation processes.
...
----
=== Resetting the simulation and processes
...
The Uniform() method uses the linear congruential generator (seed is LCGSeed, with the default value of 1878892440L) based on the algorithm in link:http://algs4.cs.princeton.edu/home/[this text], and the results of this are shuffled with the multiplicative generator (seed is MGSeed, with a default value of 772531L) as suggested by link:http://www.amazon.com/Art-Computer-Programming-Volume-Seminumerical/dp/0201896842[Maclaren and Marsaglia], to obtain a sufficiently uniform random distribution, which is then returned. As you will see later, the values of these can be changed when creating instances of various streams. However, if you do modify them then be sure you understand the implications.
...
Finally, the trigger method is invoked by the relevant TriggerQueue on the Entity. isWaiting can be called to determine whether the specific Entity is currently on a queue.
